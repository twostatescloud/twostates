...
module.exports.getKeywordCat = function(data, keyword, key) {
  var group = []
  data.forEach(function (d) {
    //for(var key in d) {
		try {
...
module.exports.initiateTableFilter = function(opts) {
  ...
  $(opts.filterDiv).keyup(function(e) {
	...
	if (text.length != 1) {
		...
		$(function() {
			$('#tvg span').miniPreview({ prefetch: 'none' });
		});
	}
  })
}
...
function searchTable(opts, searchTerm) {
  ...
  if (searchTerm.indexOf("::") > -1) {
	  ...
	  var sinceSearch = false;
		if (searchTerm[0].toLowerCase() == "added") {
			sinceSearch = true;
			var today = new Date();
			if (searchTerm[1].toLowerCase() == 'today') {
				var dateString = today.format("yyyymmdd");
				searchTerm[1] = dateString;
			} else if (searchTerm[1].toLowerCase() == 'sinceyesterday' || searchTerm[1].toLowerCase() == 'since yesterday') {
				today.setDate(today.getDate() - 1);
				var dateString = today.format("yyyymmdd");
				searchTerm[1] = dateString;
			} else if (searchTerm[1].toLowerCase().indexOf('since') > -1 && searchTerm[1].toLowerCase().indexOf('last') > -1 && searchTerm[1].toLowerCase().indexOf('days') > -1) {
				var s = parseInt(searchTerm[1].toLowerCase().replace('since','').replace('last','').replace('days','').trim());
				today.setDate(today.getDate() - s);
				var dateString = today.format("yyyymmdd");
				searchTerm[1] = dateString;
			} else {
				sinceSearch = false;
	  ...
	  var stringObject = JSON.stringify(object.channelname + " " + object.category + " " + object.country + " " + object.language + " " + object.index + " " + object.team + " " + object.verifiedon + " " + object.added + " " + object.status);
		if (searchTerm[0].toLowerCase() == "category") {
			...
			stringObject = JSON.stringify(object.added).replace('\"',"");
		...
		if (sinceSearch) {
			if (parseInt(stringObject) >= parseInt(searchTerm[1])) {
